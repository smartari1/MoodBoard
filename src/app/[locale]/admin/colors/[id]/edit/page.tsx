/**
 * Admin Color Edit Page
 * Edit existing color with professional color picker
 */
...
export default function AdminColorEditPage() {
  ...
  const tEdit = useTranslations('admin.colors.edit')
  ...
  const { data: color, isLoading, error } = useColor(colorId)
  ...
  // Load color data into form
  useEffect(() => {
    if (color) {
      reset({
        name: color.name,
        description: color.description || { he: '', en: '' },
        hex: color.hex,
  ...
  const onSubmit = async (data: UpdateColor) => {
    if (!colorId) return
    ...
    try {
      ...
      await updateMutation.mutateAsync({ id: colorId, data: submitData })
